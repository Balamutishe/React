...
router.get("/messages/:chatId", auth(), async (req, res) => {
  try {
    ...
    if (chatId) {
      ...
      const messagesList = messagesData.slice(
        (page - 1) * pageSize,
        page * pageSize
      );
      ...
      if (messagesList) {
        res
          .status(200)
          .json({ messagesList: messagesList, pageCount: pageCount });
      } else {
    ...
    try {
      ...
      if (chatId && user) {
        const statusCreate = await addMessage(req.db, {
          ...
          messageText:
            !formText || formText === "" ? "Новое сообщение" : formText,
          created_at: new Date(),
          updated_at: new Date(),
        ...
        if (!statusCreate.acknowledged) {
          ...
          const newMessage = await getOneMessage(
            req.db,
            statusCreate.insertedId
          );
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (id && messageText) {
        const statusUpdate = await updateMessage(req.db, id, {
