...
router.get("/users", auth(), async (req, res) => {
  try {
    ...
    const usersListData = await getUsers(req.db);
    const updateUsersList = usersListData
    ...
    if (usersListData) {
      res
        .status(200)
        .json({ usersList: updateUsersList, pageCount: pageCount });
    } else {
...
router.patch("/users", auth(), async (req, res) => {
  try {
    ...
    if (req.user) {
      const statusUpdate = await updateUser(req.db, req.user._id, {
        $set: userData,
      });
